...
public class TestJsonCsv {
    ...
    static void setup() throws IOException {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(jsonFile))) {
            writer.write("[");
            writer.write("{\"id\":1,\"name\":\"Kiana Lor\",\"age\":22,\"marks\":76},");
            writer.write("{\"id\":2,\"name\":\"Joshua Lonaker\",\"age\":22,\"marks\":87},");
            writer.write("{\"id\":3,\"name\":\"Dakota Blanco\",\"age\":22,\"marks\":56}");
            writer.write("]");
        }
    ...
    @AfterAll
    static void cleanup() {
        // Delete the temporary files after all tests are done
        if (jsonFile.exists()) {
            jsonFile.delete();
        }
        if (csvFile.exists()) {
    ...
    void testJsonToCsvConversion() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(csvFile))) {
            ...
            assertEquals("ID,Name,Age,Marks", header.replace("\"", ""));

            String record1 = reader.readLine();
            assertEquals("1,Kiana Lor,22,76", record1.replace("\"", ""));

            String record2 = reader.readLine();
            assertEquals("2,Joshua Lonaker,22,87", record2.replace("\"", ""));

            String record3 = reader.readLine();
            assertEquals("3,Dakota Blanco,22,56", record3.replace("\"", ""));
    ...
    void testCsvToJsonConversion() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(outputJsonFile))) {
            ...
            String expectedJson = "[{\"id\":1,\"name\":\"Kiana Lor\",\"age\":22,\"marks\":76},"
                    + "{\"id\":2,\"name\":\"Joshua Lonaker\",\"age\":22,\"marks\":87},"
                    + "{\"id\":3,\"name\":\"Dakota Blanco\",\"age\":22,\"marks\":56}]";
